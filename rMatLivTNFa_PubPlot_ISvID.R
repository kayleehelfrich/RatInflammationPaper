...
library(ggplot2)
#library(ggsignif)
setwd("C:/Users/kaylee/Documents/Kaylee_Stuff/Smith_Lab/Data/InflammationPaperFigures/MaternalLiverTNFa_ISvID")

### Reads a file in a table format and creates a data frame from it. This figures will be in the first panel.
datafile <- "E1_E2_ML_TNFa_Cq_calculations_merge_IS_ID_ForPubPlot.csv"
...
png("rMatLiv_TNFa_ISvsID_PubPlot.png", units="mm", width=100, height=120, res=600)
...
plot <- ggplot(data= dataframe_AdjCtCalcs, aes(x=Biological.Sets, y=mean), fill=order) + #creates original graph
  ...
  scale_y_continuous(limits = c(0,5.0), expand = c(0,0)) + #this sets the minimum and maximum values shown on the y-axis
  ylab(expression("Maternal Tnf-"*alpha~"/Gapdh Relative to IS-M")) + #adds y-axis label
  scale_x_discrete (limits = order) + #rearranges the x-axis in correct order
  geom_point(aes(x=Biological.Sets, y=Adj_Ct),
             size = 1.25, shape = 19, position = position_jitter(width=.2, height=0.1)) + #adds individual points
  theme(
