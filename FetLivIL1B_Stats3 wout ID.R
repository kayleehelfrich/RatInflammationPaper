...
PCR_file <- "FetLivIL1B_Ctcalcs3_For_Stats.csv"

data <- read.csv(PCR_file, header=TRUE)
data_frame <- data.frame(data)

IS_0 <- data_frame[is.element(data_frame$Biological.Sets, "IS-MD"),]
IS_5 <- data_frame[is.element(data_frame$Biological.Sets, "IS-PAE"),]
IF_0 <- data_frame[is.element(data_frame$Biological.Sets, "IF-MD"),]
...
join$groups <- ordered(join$groups, levels = c("IS_0", "IS_5", "IF_0", "IF_5"))
