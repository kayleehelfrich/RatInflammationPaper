rm(list=ls())
library("ggpubr")
#Lab desktop working directory
setwd("C:/Users/kaylee/Documents/Kaylee_Stuff/Smith_Lab/Data/InflammationPaperFigures/MaternalKidneyEPO_ISvID")
#Home laptop working directory
PCR_file <- "MatKidEPO_Ct_COMBINED_woutIF.csv"

data <- read.csv(PCR_file, header=TRUE)
data_frame <- data.frame(data)

IS_0 <- data_frame[is.element(data_frame$Biological.Sets, "IS-0"),]
IS_5 <- data_frame[is.element(data_frame$Biological.Sets, "IS-5"),]
ID_0 <- data_frame[is.element(data_frame$Biological.Sets, "ID-0"),]
ID_5 <- data_frame[is.element(data_frame$Biological.Sets, "ID-5"),]
...
tech_repl <- 5 #number of replicates
set_nums <- 3 #number of experiments
...
iron_group <- c(rep("S", reps), rep("S", reps), rep("D", reps), rep("D", reps), rep("X", reps), rep("X", reps))
alcohol_group <- c(rep("0", reps), rep("5", reps), rep("0", reps), rep("5", reps), rep("0", reps), rep("5", reps))#remove both iron and alcohol groups if say ID is not there
...
summary(model)
#running tukey test for pairwise comparisons
TukeyHSD(model)
...
#PLOTTING A MEANS PLOT - colors correspond to experiment
merge <- merge(x=join, y=data_frame, by.x = "treatments", by.y = "Cq.Averages")
colnames(merge)[3] <- "Experiment"
merge$Experiment <- as.character(merge$Experiment)

#Plotting a means plot- just one experiment
exp1 <- merge[is.element(merge$Experiment, "1"),]
ggline(exp1, x = "groups", y = "treatments", 
       add = c("mean_se", "jitter"), 
       order = c("IS_0", "IS_5", "ID_0", "ID_5", "IF_0", "IF_5"),
...
ggboxplot(join, x = "groups", y = "treatments", 
          color = "groups", palette = c("black", "red", "blue", "green", "purple", "orange"),
          order = c("IS_0", "IS_5", "ID_0", "ID_5", "IF_0", "IF_5"),
          ylab = "Cq Averages", xlab = "Treatments")
