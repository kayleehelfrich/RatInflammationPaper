rm(list=ls())
library(ggplot2)
setwd("C:/Users/kaylee/Documents/Kaylee_Stuff/Smith_Lab/Data/InflammationPaperFigures/MaternalLiverSMAD_ISvID")
PCR_file <- "MatLiv_SMAD.csv"
...
png("rMatLiv_SMAD_ISvID_PubPlot.png", units="mm", width=100, height=120, res=600) 

sets <- c("IS-M", "IS-PAE", "ID-M", "ID-PAE")
p<- ggplot(data_frame, aes(x=Treatments, y=mean, fill=sets)) + 
  ...
  scale_y_continuous(limits = c(0,200), expand = c(0,0)) + #to make the graph sit on the bottom without the space
  ...
  labs(title=("Maternal SMAD1/5 Activation"), y=("% Phosphorylation Relative to IS-M")) +
