...
library(ggplot2)
#library(ggsignif)
setwd("C:/Users/kaylee/Documents/Kaylee_Stuff/Smith_Lab/Data/InflammationPaperFigures/FetalLiverEPO_ISvID")

### Reads a file in a table format and creates a data frame from it. This figures will be in the first panel.
datafile <- "FetLivEPO_Ctcalcs3_Combined_ForPubPlot.csv"
...
png("rFetLiv_EPO_ISvsID_PubPlot.png", units="mm", width=100, height=120, res=600)
...
plot <- ggplot(data= dataframe_AdjCtCalcs, aes(x=Biological.Sets, y=mean), fill=order) + #creates original graph
  ...
  ylab(expression("Fetal Epo/Gapdh Relative to IS-M")) + #adds y-axis label
