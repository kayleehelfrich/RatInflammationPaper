...
library(ggplot2)
#library(ggsignif)
setwd("C:/Users/kaylee/Documents/Kaylee_Stuff/Smith_Lab/Data/InflammationPaperFigures/MaternalLiverIL1B_ISvIF")

### Reads a file in a table format and creates a data frame from it. This figures will be in the first panel.
datafile <- "E1_E2_ML_IL-1b_Cq_calculations_merge_IS_IF_ForPubPlot.csv"
...
png("rMatLiv_IL1B_ISvsIF_PubPlot.png", units="mm", width=100, height=120, res=600)

# Sets order of samples for future steps
order <- c("IS-M", "IS-PAE", "IF-M", "IF-PAE")
...
plot <- ggplot(data= dataframe_AdjCtCalcs, aes(x=Biological.Sets, y=mean), fill=order) + #creates original graph
  ...
  ylab("Maternal Il-1"*beta~"/Gapdh Relative to IS-M") + #adds y-axis label
  ...
  theme(
    plot.title = element_text(hjust = .5, size = 18, face = "italic"), #sets size and position of graph title
