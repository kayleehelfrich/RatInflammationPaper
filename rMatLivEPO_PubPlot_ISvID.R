...
datafile <- "MatKidEPO_Ct_COMBINED_ForPubPlot.csv"
...
png("rMatKid_EPO_ISvsID_PubPlot.png", units="mm", width=100, height=120, res=600)
...
plot <- ggplot(data= dataframe_AdjCtCalcs, aes(x=Biological.Sets, y=mean), fill=order) + #creates original graph
  ...
  ylab(expression("Maternal Kidney Epo/Gapdh Relative to IS-M")) + #adds y-axis label
