...
library(ggplot2)
#library(ggsignif)
setwd("C:/Users/kaylee/Documents/Kaylee_Stuff/Smith_Lab/Data/InflammationPaperFigures/MaternalLiverIL6_ISvIF")

### Reads a file in a table format and creates a data frame from it. This figures will be in the first panel.
datafile <- "E1_E2_E3_ML_IL-6_Cq_calculations_merge_IS_IF_ForPubPlot.csv"
...
png("rMatLiv_IL6_ISvsIF_PubPlot.png", units="mm", width=100, height=120, res=600)
...
plot <- ggplot(data= dataframe_AdjCtCalcs, aes(x=Biological.Sets, y=mean), fill=order) + #creates original graph
  ...
  scale_y_continuous(limits = c(0,12.0), expand = c(0,0)) + #this sets the minimum and maximum values shown on the y-axis
  ylab(expression('Maternal Il-6/Gapdh Relative to IS-M')) + #adds y-axis label
