...
library(ggplot2)
#library(ggsignif)
setwd("C:/Users/kaylee/Documents/Kaylee_Stuff/Smith_Lab/Data/InflammationPaperFigures/MaternalLiverIL10_ISvIDvIF")

### Reads a file in a table format and creates a data frame from it. This figures will be in the first panel.
datafile <- "E2_E3_ML_IL-10_Cq_merge_For_PubPlot.csv"
...
png("rMatLiv_IL10_ISvsIDvIF_PubPlot.png", units="mm", width=150, height=140, res=600)

# Sets order of samples for future steps
order <- c("IS-M", "IS-PAE", "ID-M", "ID-PAE", "IF-M", "IF-PAE")
...
plot <- ggplot(data= dataframe_AdjCtCalcs, aes(x=Biological.Sets, y=mean), fill=order) + #creates original graph
  ...
  ggtitle("Maternal Liver Il-10 Expression") +
  ...
  scale_y_continuous(limits = c(0,4.0), expand = c(0,0)) + #this sets the minimum and maximum values shown on the y-axis
  ylab(expression("Maternal Il-10/Gapdh Relative to IS-M")) + #adds y-axis label
  ...
  theme(
    ...
    axis.text=element_text(size=17, color = "black")) #sets size of x and y axis labels
