...
png("rFetLiv_TNFa_ISvsIF_ddCtDotPlot2.png", units="mm", width=120, height=120, res=600)
...
p <- ggplot(data= dataframe_AdjCtCalcs, aes(x=Biological.Sets, y=Adj_Ct), fill=order) + #creates original graph
  ...
  ggtitle("TNFa Expression") + #adds title
  #xlab("Treatment") + #adds x-axis label
  ylab("Fetal Liver TNFa/Gapdh Relative to IS-MD") + #adds y-axis label
