...
library(ggplot2)
#library(ggsignif)
setwd("C:/Users/kaylee/Documents/Kaylee_Stuff/Smith_Lab/Data/InflammationPaperFigures/MaternalLiverIFNy_ISvIF")

### Reads a file in a table format and creates a data frame from it. This figures will be in the first panel.
datafile <- "E1_E2_ML_IFNg_Cq_calculations_merge_IS_IF_ForPubPlot.csv"
...
png("rMatLiv_IFNy_ISvsIF_PubPlot.png", units="mm", width=100, height=120, res=600)
...
plot <- ggplot(data= dataframe_AdjCtCalcs, aes(x=Biological.Sets, y=mean), fill=order) + #creates original graph
  ...
  ylab("Maternal Ifn-"*gamma~"/Gapdh Relative to IS-M") + #adds y-axis label
