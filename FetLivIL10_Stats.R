rm(list=ls())
library("ggpubr")
setwd("C:/Users/kaylee/Documents/Kaylee_Stuff/Smith_Lab/Data/InflammationPaperFigures/FetalLiverIL10_ISvIDvIF")
PCR_file <- "FetLivIL10_Ctcalcs3_Combined_For_PubPlot.csv"
...
sets$IF_0 <- IF_0$Cq.Averages
sets$IF_5 <- IF_5$Cq.Averages
colnames(sets)[1] <- "IS_0" #renaming the first column

treatments <- c(sets$IS_0, sets$IS_5, sets$ID_0, sets$ID_5, sets$IF_0, sets$IF_5)
...
groups <- c(rep("IS_0", reps), rep("IS_5", reps), rep("ID_0", reps), rep("ID_5", reps), rep("IF_0", reps), rep("IF_5", reps))
...
join$groups <- ordered(join$groups, levels = c("IS_0", "IS_5", "ID_0", "ID_5", "IF_0", "IF_5"))
